...
class TTFMAKE_Builder_Base {
	...
	public function __construct() {
		...
		require_once make_get_plugin_directory() . '/inc/builder/core/configuration-helpers.php';

		// Include the save routines
		require_once make_get_plugin_directory() . '/inc/builder/core/save.php';
		...
		// HappyForms ad
		require_once make_get_plugin_directory() . '/inc/happyforms.php';
	...
	public function display_builder_toggle() {
		...
		$label = 'page' === get_post_type( $post_id ) ?
				esc_html__( 'Enable page builder', 'make' ) :
				esc_html__( 'Enable post builder', 'make' );
	?>
		<p>
	...
	public function display_builder( $post_local ) {
		...
		require_once( make_get_plugin_directory() . '/inc/builder/core/templates/menu.php' );
		require_once( make_get_plugin_directory() . '/inc/builder/core/templates/stage.php' );
	}
	...
	public function admin_enqueue_scripts( $hook_suffix ) {
		...
		wp_enqueue_style(
			'ttfmake-builder',
			make_get_plugin_directory_uri() . 'css/builder/core/builder.css',
			array(),
			TTFMAKE_VERSION
		);
		...
		wp_enqueue_script(
			'ttfmake-builder',
			make_get_plugin_directory_uri() . 'js/builder/core/builder.js',
			$dependencies,
			TTFMAKE_VERSION,
			true
		);
		...
		wp_enqueue_script(
			...
			make_get_plugin_directory_uri() . 'js/builder/core/overlay.js',
		...
		$builder_settings = apply_filters( 'make_builder_settings', $builder_settings );
		...
		wp_enqueue_script(
			...
			make_get_plugin_directory_uri() . 'js/builder/core/edit-page.js',
			array( 'jquery' ),
			TTFMAKE_VERSION,
			true
		);
		...
		if ( 'page' === get_post_type( get_the_ID() ) ) {
			...
			$layout = get_post_meta( get_the_ID(), '_ttfmake_layout', true );
...
?>
	...
	public function print_templates() {
		...
		$ttfmake_overlay_class = 'ttfmake-overlay ttfmake-content-overlay';
		$ttfmake_overlay_title = __( 'Edit content', 'make' );
		$ttfmake_overlay_button_label = __( 'Update changes', 'make' );
		require_once( make_get_plugin_directory() . '/inc/builder/core/templates/overlay-tinymce.php' );

		// Configuration overlay templates
		$ttfmake_overlay_id = '{{ data.id }}';
		$ttfmake_overlay_class = '{{ data.className }}';
		$ttfmake_overlay_title = '{{ data.title }}';
		$ttfmake_overlay_button_label = '{{ data.buttonLabel }}';
		require_once( make_get_plugin_directory() . '/inc/builder/core/templates/overlay-configuration.php' );
		?>
...
function ttfmake_will_be_builder_page() {
	 ...
	 */
	return apply_filters( 'make_will_be_builder_page', 1 === $use_builder, '', $use_builder );
}
endif;
...
function ttfmake_load_section_header() {
	...
	require( make_get_plugin_directory() . '/inc/builder/core/templates/section-header.php' );
	 ...
	 */
	if ( isset( $ttfmake_section_data['id'] ) ) {
...
function ttfmake_load_section_footer() {
	...
	require( make_get_plugin_directory() . '/inc/builder/core/templates/section-footer.php' );
