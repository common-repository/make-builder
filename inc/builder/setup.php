...
class MAKE_Builder_Setup extends MAKE_Util_Modules implements MAKE_Builder_SetupInterface, MAKE_Util_HookInterface {
	...
	public function __construct( MAKE_APIInterface $api, array $modules = array() ) {
		...
		// Include the API
		require_once make_get_plugin_directory() . '/inc/builder/core/api.php';

		// Load backend files
		if ( is_admin() ) {
			require_once make_get_plugin_directory() . '/inc/builder/core/base.php';
		}
	}
	...
	public function display_builder_toggle_upsell() {
		...
		<p style="color: #666;">
			<em><?php _e( 'Did you know: with Make Plus you can use Make Builder on posts, too.', 'make'  ) ?></em>
		<p>
		<p>
			<a href="<?php echo esc_url( 'https://thethemefoundry.com/wordpress-plugins/make-builder/?utm_source=app&utm_campaign=post-builder#get-started' ); ?>" target="_blank" class="button button-primary button-large"><?php _e( 'Upgrade to Make Plus Now', 'make' ); ?></a>
		</p>
		<?php
	}
			...
			foreach ( $section_types as $section_id => $section_type ) {
				switch ( $section_type ) {
					...
					case 'productgrid' :
						...
						if ( !defined( 'MAKE_LOAD_CYCLE2_DEPENDENCY' ) || MAKE_LOAD_CYCLE2_DEPENDENCY !== false ) {
...
function ttfmake_is_builder_page( $post_id = 0 ) {
	...
	$has_builder_meta = get_post_meta( $post_id, '_ttfmake_layout', true );
